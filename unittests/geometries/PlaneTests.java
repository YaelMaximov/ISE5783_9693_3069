...
class PlaneTests {
    ...
    void testFindIntersectionPoints() throws IllegalAccessException {
        ...
        assertEquals(expected, plane.findIntersections(ray),"wrong point for regular ray that crosses the plane");
        ...
        assertNull(plane.findIntersections(ray),"regular ray that doesn't cross the plane shouldn't returned value");

        // =============== Boundary Values Tests ==================
        // **** Group: Ray's line parallel to the plane
        // TC11: Ray in the plane (0 points)
        ray = new Ray(new Point(3,2,1), new Vector(0, 0, 1));
        assertNull(plane.findIntersections(ray),"ray in the plane shouldn't returned value");

        // TC12: Ray out of the plane (0 points)
        ray = new Ray(new Point(2, 1, 2), new Vector(1, 0, 0));
        assertNull(plane.findIntersections(ray),"ray parallel to a plane but not in it shouldn't return value");
        ...
        assertEquals(expected, plane.findIntersections(ray),"wrong value for the cross point of an orthogonal ray begins before the plane");

        // TC14: Ray starts in the plane (0 points)
        ray = new Ray(new Point(3, 2, 1), new Vector(0, 1, 0));
        assertNull(plane.findIntersections(ray),"orthogonal ray begins in the plane shouldn't return value");

        // TC15: Ray starts after the plane (0 points)
        ray = new Ray(new Point(2, 3, 2), new Vector(0, 1, 0));
        assertNull(plane.findIntersections(ray),"orthogonal ray after in the plane shouldn't return value");
        ...
        assertNull(plane.findIntersections(ray),"regular ray starts in the plane shouldn't return value");
        ...
        assertNull(plane.findIntersections(ray),"regular ray starts in the point defining the  plane shouldn't return value");
    }
